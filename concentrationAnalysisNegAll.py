...
def main():
    ...
    for imageName in testImages:
        ...
        print('number of cluster centers: '+ str(len(coordCentersHighestClass)))
        ...
        # kdt pair-wise distance calculation for random points
        # X = np.array([[pixelCoord.x,pixelCoord.y] for pixelCoord in randomPointList])
        # kdt = KDTree(X, leaf_size=30, metric='euclidean')
        # queryResultsAllRandom = kdt.query(X, k=len(randomPointList), return_distance=True)
        # distanceArrayAllRandom = queryResultsAllRandom[0] # distance
        # distanceListAllRandomList = distanceArrayAllRandom.tolist()
        # medianDistanceListAllRandomList = [statistics.median(distanceAllRandom) for distanceAllRandom in distanceListAllRandomList]
        
        # randomSampleArray = np.asarray(medianDistanceListAllRandomList)
        # mapSampleArray = np.asarray(medianDistanceListCoordCenters)
        # identicalResult = ranksums(mapSampleArray, randomSampleArray)
        # lessResult = ranksums(mapSampleArray, randomSampleArray,alternative = 'less')
        # greaterResult = ranksums(mapSampleArray, randomSampleArray,alternative = 'greater')

        # print(identicalResult)
        # print(lessResult)
        # print(greaterResult)
