...
def main():
	...
	for i, imageName in enumerate(testImages):
		...
		if imageName == 'us_Blues_4_neg.jpg':
			afterTarget = True
		...
		for i, spbox in enumerate(spShapelyBoxList):
			isInMapBbox = bboxMap.contains(spbox) or bboxMap.intersects(spbox)
			if isInMapBbox:
				print(i)
		...
		for sp in superPixelList:
			...
			if (sp.isInMapBbox) and abs(sp.maxOccurValue - bgColorValue) > 5:
		...
		for mapSP in mapSuperpixelList:
			...
			xMax = int(bounds[2])
			yMax = int(bounds[3])
			croppedImg = image[yMin:yMax,xMin:xMax]
			dominantColor = unique_count_app(croppedImg)
			dominantColorGrey = int(np.dot(dominantColor, rgb_weights))
			...
			if  abs(dominantColorGrey - bgColor) > 5:
				if all(diffList):
					...
					for ind in sameGreyIndex:
						...
						if abs(espR - dcR) <= 10 and abs(espG - dcG) <= 10 and abs(espG - dcG) <= 10:
							rgbDiff = False
							break
					if rgbDiff:
						superPixelGreyValueList.append(dominantColorGrey)
